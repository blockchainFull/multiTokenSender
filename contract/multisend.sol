...
contract Mutlisender is Ownable {
    ...
    uint256 public fee;
    uint256 public feeVip1;
    uint256 public feeVip2;
    uint256 public feeVip3;
    uint256 public feeVip4;
    ...
    function setVip2Fee(uint256 newFee) public onlyOwner {
        feeVip2 = newFee;
    }
    
    function setVip3Fee(uint256 newFee) public onlyOwner {
        feeVip3 = newFee;
    }
    
    function setVip4Fee(uint256 newFee) public onlyOwner {
        feeVip4 = newFee;
    }
    
    function setVipLevel() public payable {
        require(msg.value >= feeVip1, "Unsuitable Amount");
        
        if (msg.value < feeVip2) {
            vipLevel[msg.sender] = 1;
        } else if (msg.value < feeVip3) {
            vipLevel[msg.sender] = 2;
        } else if (msg.value < feeVip4) {
    ...
    function multisendEther(address payable[] memory _contributors, uint256[] memory _balances) public payable {
        ...
        uint256 feeForSend = currentFee(msg.sender);
        require(total >= feeForSend);
        require(_contributors.length <= arrayLimit);
        total = total.sub(feeForSend);
        uint256 i = 0;
        for (i; i < _contributors.length; i++) {
    ...
    function claimTokens(address _token) public onlyOwner {
        ...
        ClaimedTokens(_token, mainWallet, balance);
    }
